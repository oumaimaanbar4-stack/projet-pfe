...
export default function Clients() {
  ...
  const handleBulkDelete = async () => {
    try {
      await Promise.all(selectionModel.map(id => api.delete(`/clients/${id}`)));
      setRows(prev => prev.filter(r => !selectionModel.includes(r.id)));
      toast(`${selectionModel.length} clients supprimés`, 'info');
      setSelectionModel([]);
    } catch {
      toast('Erreur lors de la suppression', 'error');
    }
  ...
  const columns = [
      ...
      renderCell: ({ row }) => (
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          ...
          <Box>
            ...
            <Typography sx={{ fontSize: '0.7rem', color: '#94a3b8' }}>{row.adresse || '—'}</Typography>
          </Box>
        </Box>
      ),
    },
    {
      ...
      renderCell: ({ value }) => <Typography sx={{ fontSize: '0.8rem', color: '#3b82f6' }}>{value}</Typography>,
    },
    {
    ...
    },
    {
      ...
      renderCell: ({ value }) => (
        <Chip label={`${value || 0} fact.`} size="small"
          sx={{ bgcolor: '#eff6ff', color: '#1d4ed8', fontWeight: 700, fontSize: '0.68rem', border: 'none' }} />
      ),
    },
    {
      field: 'created_at', headerName: 'Ajouté le', width: 130,
      renderCell: ({ value }) => (
    ...
    <Box sx={{ bgcolor: '#f8fafc', minHeight: '100vh' }}>
      ...
      <Box sx={{ p: { xs: 2, md: 4 }, maxWidth: '1600px', margin: '0 auto' }}>
        ...
        <Box sx={{ bgcolor: 'white', border: '1px solid #e2e8f0', borderRadius: 3, borderBottomLeftRadius: 0, borderBottomRightRadius: 0, p: 2, display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap' }}>
          <TextField placeholder="Rechercher un client…" value={search} onChange={e => setSearch(e.target.value)} size="small"
          ...
          {selectionModel.length > 0 && (
            <Button startIcon={<DeleteOutlineRoundedIcon />} onClick={handleBulkDelete} size="small"
              sx={{ ml: 'auto', color: '#ef4444', fontWeight: 700, textTransform: 'none', fontSize: '0.8rem', border: '1px solid #fee2e2', bgcolor: '#fff5f5', borderRadius: '8px', px: 2, '&:hover': { bgcolor: '#fee2e2' } }}>
        ...
        <Box sx={{ bgcolor: 'white', border: '1px solid #e2e8f0', borderTop: 'none', borderRadius: 3, borderTopLeftRadius: 0, borderTopRightRadius: 0, overflow: 'hidden' }}>
          <DataGrid
            rows={filtered} columns={columns} autoHeight loading={loading}
        ...
        {form.nom.trim() && (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, px: 3, pt: 3 }}>
            ...
            <Box>
              ...
              <Typography sx={{ fontSize: '0.72rem', color: '#94a3b8' }}>{form.societe}</Typography>
            </Box>
          </Box>
        )}
        ...
        <Box sx={{ px: 3, py: 3, display: 'flex', flexDirection: 'column', gap: 2.5, overflowY: 'auto', flex: 1 }}>
          <Box>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <TextField label="Société" value={form.societe} onChange={e => setForm(f => ({ ...f, societe: e.target.value }))}
                fullWidth size="small" sx={{ '& .MuiOutlinedInput-root': { borderRadius: '8px' } }} />
              <TextField label="Email *" type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))}
                error={!!formErrors.email} helperText={formErrors.email}
                fullWidth size="small" sx={{ '& .MuiOutlinedInput-root': { borderRadius: '8px' } }} />
              <TextField label="Téléphone *" value={form.telephone} onChange={e => setForm(f => ({ ...f, telephone: e.target.value }))}
                error={!!formErrors.telephone} helperText={formErrors.telephone}
                fullWidth size="small" sx={{ '& .MuiOutlinedInput-root': { borderRadius: '8px' } }} />
              <TextField label="Adresse" value={form.adresse} onChange={e => setForm(f => ({ ...f, adresse: e.target.value }))}
                fullWidth size="small" sx={{ '& .MuiOutlinedInput-root': { borderRadius: '8px' } }} />
            </Box>
          </Box>
          ...
          <Box>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <TextField label="Numéro TVA" value={form.numero_tva} onChange={e => setForm(f => ({ ...f, numero_tva: e.target.value }))}
                fullWidth size="small" sx={{ '& .MuiOutlinedInput-root': { borderRadius: '8px' } }} />
      ...
      <Dialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)}
        ...
        <DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
          ...
          <Button onClick={() => handleDelete(deleteTarget.id)} variant="contained"
            sx={{ borderRadius: '8px', textTransform: 'none', fontWeight: 700, bgcolor: '#ef4444', boxShadow: 'none', '&:hover': { bgcolor: '#dc2626', boxShadow: 'none' } }}>
            Supprimer
          </Button>
        </DialogActions>
      </Dialog>
